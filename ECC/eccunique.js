...
// Création d'un nouvel élément du champ
...
// Choix d'un élément aléatoire du champ
function randFieldElem(field) {
	if (field == 'prime') {
		return randBigInt(100).mod(p28);
	} else {
		return randOEF();
	}
}
...
var h26 = nbi();
var h27 = nbi();
var inv2 = noef(2).modInverse(); // précalcul de l'inverse de 2
var hh1 = new OEF();
var hh2 = new OEF();
var hh3 = new OEF();
var hh4 = new OEF();
...
function ecPoint(coord, curve, rep, field, m, inf) {//
	this.inf = inf; // indique si c'est le point à l'infini
	this.coord = coord; // tableau des coordonnées
	this.rep = rep;
	this.curve = curve;
	this.field = field;
	this.m = m;
...
// Conversion en jacobien
function ecpJacobian() {
	if (this.rep == 'jacobian')
		return this.copy();
	else {
		if (this.inf) {
			var r = infinity(this);
		...
		// ~ this.rep='jacobian';
		return new ecPoint(
				[ this.coord[0], this.coord[1], fieldONE(this.field) ],
				this.curve, 'jacobian', this.field, this.m, 0);
	}
}

// Conversion en affine
function ecpAffine() {
	if (this.rep == 'affine')
		return this.copy();
	else {
		...
		var invZ3 = this.coord[2].multiply(this.coord[2]).multiply(
				this.coord[2]).modInverse(this.m);
		var X = this.coord[0].multiply(invZ2).mod(this.m);
		var Y = this.coord[1].multiply(invZ3).mod(this.m);
		// ~ this.coord=[X,Y];
		// ~ this.rep='affine';
		return new ecPoint([ X, Y ], this.curve, 'affine', this.field, this.m,
				0);
	}
}
...
function ecpAddOEF(S) {
	...
	;// coord=='inf') {document.write('hello1');return this;}
	if (this.inf) {
		return S.jacobian();
	}
	;// coord=='inf')
	// {document.write('hello');document.write(this.coord);return
	// S.jacobian();document.write('hello');}
	var X1 = this.coord[0];
	var Y1 = this.coord[1];
	var Z1 = this.coord[2];
	var x2 = S.coord[0];
...
function ecpDobbleP() {// X,Y,Z,p){
	...
	y3 = y3.square().spMod(p, h11).multiply(
			p.shiftRight(1).spAdd(BigInteger.ONE, h19)).spMod(p, h12);
...
function ecpAddP(S) {
	...
	if (t1.isZero()) {
		if (t2.isZero()) {// document.write('special case');
			return S.dobble();
		}// pointDoubling(x2,y2,BigInteger.ONE); }// changer le 1
		else {
			return infinity(this);
		}
	}
...
function ecpAddPJJ(S) {
	// idem ecpAddP mais addition en coordonnees Jacobiennes
	// The curve must be of the form Y=X^3-3X+b
	// Source:
    ...
    x3 = ((r.spAdd(r,h15)).spSubtract(j,h16)).spSubtract( (v.spAdd(v,h18)) ,h17);
    t2 = s1.multiply(j).spMod(p,h19);
    t3 = t2.spAdd(t2,h20);
    y3 = (r.multiply(v.spSubtract(x3,h21)).spMod(p,h22)).spSubtract(t2,h23); 
    t4 = (Z1.spAdd(Z2,h24)).square().spMod(p,h25);
    z3 = ((t4.spSubtract(z1z1,h26)).spSubtract(z2z2,h27)).multiply(h).spMod(p,h26);
...
/**
 * XXX multiplication utilisant la table précalculée
 */
...
function precomputeComb2(k, P, w) {
	...
	for ( var ipc = 1; ipc < r; ipc++) {
		...
		precomp[ipc] = precomp[ipc - (1 << (fact - 1))].add(tab[fact - 1])
				.affine();
		precomp[ipc + r] = precomp[ipc + r - (1 << (fact - 1))].add(
				tab[w + fact - 1]).affine();
	}
	return precomp;
}

function precomputeComb2Jacob(k, P, w) {
	...
	var tab = new Array();
	tab[0] = R;
	tab[w] = R.dobbleMult(e);// .jacobian();
	for ( var i = 1; i < w; i++) {
		tab[i] = tab[i - 1].dobbleMult(d);// .affine();
		tab[i + w] = tab[i].dobbleMult(e);// .affine();
		// Les doublages se font en J*J -> J, on évite une conversion
	}
	var precomp = new Array();
	precomp[0] = infinity(R);
	precomp[r] = infinity(R);
	...
	// La table de précalcul est totalement en jacobien
	return precomp;
}
...
function readPrecompOEF(Px, Py, P) {
	...
	for (i = 1; i < nb; i++) {
		precomp[i] = new ecPoint([ noeftab(Px.slice(i * 10, i * 10 + 10)),
				...
				'OEF', 0, 0);// [noeftab64(Px.slice(i*8,i*8+8)),noeftab64(Py.slice(i*8,i*8+8))],'curve','affine','OEF',0,0);
	}
...
function displayPrecompP(precomp) {
	...
	// ~ document.write("new
	// BigInteger('"+precomp[index].coord[0].toString(35)+"',35)");
	// ~ document.write("new
	// BigInteger('"+precomp[index].coord[1].toString(35)+"',35)");
...
function ecAdd(Q) {
	// XXX Modification du support des J + J -> J
	if (this.rep == 'jacobian') {
		if (Q.rep == 'jacobian') {
			if (this.field == 'prime') {
				return this.pAddPJJ(Q);
			}
		}
	}
...
ecPoint.prototype.pAddPJJ = ecpAddPJJ;
...
ecPoint.prototype.copy = function() {
